...
// ============================================
// Auto-generate a "Data Inventory" block so the AI knows exactly what it received
function buildDataInventory(data: any): string {
    const available: string[] = [];
    const missing: string[] = [];

    // Core data
    if (data.campaigns?.length > 0) available.push(`Campaigns: ${data.campaigns.length}`);
    else missing.push('Campaigns');
    if (data.adGroups?.length > 0) available.push(`Ad Groups: ${data.adGroups.length}`);
    if (data.keywords?.length > 0) available.push(`Keywords: ${data.keywords.length}`);
    if (data.ads?.length > 0) available.push(`Ads: ${data.ads.length}`);
    if (data.negativeKeywords?.length > 0) available.push(`Negative Keywords: ${data.negativeKeywords.length}`);
    if (data.searchTerms?.length > 0) available.push(`Search Terms: ${data.searchTerms.length}`);

    // Context signals
    if (data.contextBlock) {
        if (data.contextBlock.includes('Device') || data.contextBlock.includes('Устройства')) available.push('Device Split');
        if (data.contextBlock.includes('Peak Hours') || data.contextBlock.includes('Пикови часове')) available.push('Hour of Day');
        if (data.contextBlock.includes('Day of Week') || data.contextBlock.includes('Ден от седмицата')) available.push('Day of Week');
        if (data.contextBlock.includes('Top Regions') || data.contextBlock.includes('Топ региони')) available.push('Geo Split');
        if (data.contextBlock.includes('Competitive Pressure') || data.contextBlock.includes('Конкурентен натиск')) available.push('Auction Insights');
        if (data.contextBlock.includes('Landing Page') || data.contextBlock.includes('Landing Pages')) available.push('Landing Page Health');
        if (data.contextBlock.includes('Conversion Actions') || data.contextBlock.includes('Конверсионни действия')) available.push('Conversion Actions');
    } else {
        missing.push('Device Split', 'Hour/Day', 'Geo', 'Auction Insights', 'Landing Pages', 'Conversion Actions');
    }

    // PMax context
    if (data.pmaxBlock) {
        if (data.pmaxBlock.includes('Asset Groups')) available.push('PMax Asset Groups');
        if (data.pmaxBlock.includes('Asset Inventory') || data.pmaxBlock.includes('инвентар')) available.push('PMax Asset Inventory');
        if (data.pmaxBlock.includes('Search Categories') || data.pmaxBlock.includes('търсени категории')) available.push('PMax Search Categories');
    }

    // Pre-analysis
    if (data.nGramAnalysis) available.push('N-Gram Analysis');

    const lines = [`=== DATA INVENTORY (auto-generated — do NOT contradict) ===`];
    lines.push(`AVAILABLE: ${available.join(', ')}`);
    if (missing.length > 0) lines.push(`NOT PROVIDED: ${missing.join(', ')}`);
    lines.push(`RULE: Do NOT claim any "available" data is missing. Do NOT fabricate data that is "not provided".`);
...
function buildPrompt(data: any): string {
    ...
    const dataInventory = buildDataInventory(data);
